...
$_tr = mysqli_query($conn, "SELECT * FROM teachers WHERE id=$tid LIMIT 1");
if (!$_tr || !($teacher = mysqli_fetch_assoc($_tr))) {
    session_destroy(); header('Location: login.php?err=account'); exit;
}
...
/* Nav badge counts */
$nav_subj   = (int)(mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM teacher_subject_assignments WHERE teacher_id=$tid"))[0] ?? 0);
$nav_quiz   = (int)(mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM teacher_quizzes WHERE teacher_id=$tid AND status='active'"))[0] ?? 0);
$nav_assign = (int)(mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM teacher_assignments WHERE teacher_id=$tid AND status='active'"))[0] ?? 0);
$nav_tests  = (int)(mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM teacher_tests WHERE teacher_id=$tid AND status='scheduled'"))[0] ?? 0);

$words      = array_values(array_filter(explode(' ', $teacher['name']), fn($w) => ctype_alpha($w[0] ?? '')));
$initials   = strtoupper(($words[0][0] ?? '') . ($words[1][0] ?? ''));
$first_name = htmlspecialchars($words[0] ?? $teacher['name']);
$nav        = $active_nav ?? '';
$photo_src  = !empty($teacher['photo']) ? '../assets/uploads/teachers/' . htmlspecialchars($teacher['photo']) : '';
$desig      = htmlspecialchars($teacher['designation'] ?: 'Teacher');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title><?= htmlspecialchars($page_title ?? 'Dashboard') ?> — Teacher Portal</title>
...
.bs-normal { background: #f4f7fb; color: #6c7a8d; }
.bs-important { background: #fff8e6; color: #92400e; }
.bs-urgent { background: #fff0f0; color: #991b1b; }
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sb-profile">
    ...
    <div style="min-width:0">
      <div class="sb-pname"><?= htmlspecialchars($teacher['name']) ?></div>
      <div class="sb-pid"><?= htmlspecialchars($teacher['teacher_username'] ?? '') ?></div>
      <span class="sb-badge"><i class="fa-solid fa-circle-check"></i> Active</span>
    </div>
  </div>
  <nav class="sb-nav">
    <div class="sb-section-label">Main</div>
    <a href="portal.php" class="sb-link <?= $nav==='dashboard'?'active':'' ?>">
    ...
    <a href="announcements.php" class="sb-link <?= $nav==='announcements'?'active':'' ?>">
      <i class="fa-solid fa-bullhorn"></i> Announcements
    </a>
    <div class="sb-section-label">Account</div>
    <a href="profile.php" class="sb-link <?= $nav==='profile'?'active':'' ?>">
      <i class="fa-solid fa-circle-user"></i> My Profile
    </a>
    <a href="change_password.php" class="sb-link <?= $nav==='change_password'?'active':'' ?>">
      ...
      <div class="dropdown">
        <button class="hdr-user dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
          <div class="hdr-avatar">
            <?php if ($photo_src): ?>
              <img src="<?= $photo_src ?>" alt="" onerror="this.style.display='none';this.parentElement.textContent='<?= $initials ?>'">
            <?php else: ?>
              <?= $initials ?>
            <?php endif; ?>
          </div>
