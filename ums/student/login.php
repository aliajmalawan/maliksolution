...
if (($_GET['switch'] ?? '') === '1') { ums_logout(); redirect(UMS_URL . '/student/login.php'); }

$switchUser = null;
if ($u = ums_user()) {
    if ($u['role'] === 'student') { redirect(UMS_URL . '/student/portal.php'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check()) {
        ...
        $error    = ums_attempt_login($email, $password, ['student']) ?? '';
        if ($error === '') { redirect(UMS_URL . '/student/portal.php'); }
    }
}
?>
<!DOCTYPE html>
...
<title>Student Portal — <?= e(UMS_NAME) ?></title>
...
<div class="login-card">
  <div class="login-logo">
    <div class="login-logo-icon"><i class="fa-solid fa-user-graduate"></i></div>
    <h1>Student Portal</h1>
    <p><?= e(UMS_NAME) ?> — Student System</p>
  </div>
  ...
  <div class="alert alert-info py-2 px-3 mb-3" style="font-size:.83rem;border-radius:10px">
    ...
    <a href="?switch=1" style="font-weight:700">Log out</a> to sign in as a student instead.
  </div>
  <?php endif; ?>
  ...
  <form method="POST">
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        ...
        <input type="email" name="email" class="form-control" placeholder="you@students.ums.local"
