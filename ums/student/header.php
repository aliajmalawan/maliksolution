...
/** Student Portal shell — expects $page_title, $active_nav. Close with footer.php. */

$words      = array_values(array_filter(explode(' ', $student['name']), fn($w) => ctype_alpha($w[0] ?? '')));
$initials   = strtoupper(($words[0][0] ?? '') . ($words[1][0] ?? ''));
$first_name = e($words[0] ?? $student['name']);
$nav        = $active_nav ?? '';
$photo_src  = $student['photo'] !== '' ? UMS_URL . '/' . e($student['photo']) : '';
$prog       = e($student['program'] ?: 'Student');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title><?= e($page_title ?? 'Dashboard') ?> — Student Portal</title>
...
.dl-item, .ds-item { display: flex; align-items: center; gap: .7rem; padding: .7rem 0; border-bottom: 1px solid var(--line); }
.dl-item:last-child, .ds-item:last-child { border-bottom: none; }
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sb-profile">
    ...
    <div style="min-width:0">
      <div class="sb-pname"><?= e($student['name']) ?></div>
      <div class="sb-pid"><?= e($student['registration_no']) ?></div>
  ...
  <nav class="sb-nav">
    ...
    <a href="portal.php#sec-subjects" class="sb-link"><i class="fa-solid fa-book-open"></i> My Subjects</a>
