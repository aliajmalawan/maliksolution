...
if ($action === 'profile_save') {
    ...
    $stmt = $db->prepare('UPDATE ' . tbl('teachers') . ' SET email=?, phone=?, address=? WHERE id=?');
    $stmt->bind_param('sssi', $email, $phone, $address, $tid);
    $stmt->execute(); $stmt->close();

    ums_log('teacher_profile_update', 'Updated own profile');
    flash_set('success', 'Profile updated.');
    redirect(tp_url('profile.php'));
}
...
if ($action === 'change_password') {
    ...
    if (!$login || !password_verify($cur, $login['password_hash'])) {
        ...
        redirect(tp_url('change_password.php'));
    }
    if (strlen($new1) < 6) {
    ...
    ums_log('teacher_password_change', 'Changed own password');
