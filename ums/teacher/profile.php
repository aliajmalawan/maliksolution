...
$deptName = '';
if ((int)$teacher['department_id'] > 0) { $deptName = dept_options((int)$authUser['campus_id'])[(int)$teacher['department_id']] ?? ''; }
$login = tch_login_find($tid);
...
<div class="page-hd">
  <div><h1><i class="fa-solid fa-user-circle me-2 text-primary"></i>My Profile</h1><p>Your professional record and contact details.</p></div>
</div>
...
<div class="row g-3">
  <div class="col-lg-4">
    <div class="cardx text-center">
      <?php if ($teacher['photo'] !== ''): ?>
        <img src="<?= UMS_URL . '/' . e($teacher['photo']) ?>" style="width:110px;height:110px;border-radius:50%;object-fit:cover;object-position:center 15%;border:3px solid var(--line);margin-bottom:1rem">
      <?php else: ?>
      ...
      <?php endif; ?>
      <h4 style="font-size:1.05rem;font-weight:800;color:var(--navy)"><?= e($teacher['name']) ?></h4>
      <p class="text-muted" style="font-size:.82rem"><?= e($teacher['designation'] ?: '—') ?></p>
      <hr>
      <div class="text-start" style="font-size:.82rem">
        <div class="mb-2"><strong>Employee No:</strong> <code><?= e($teacher['employee_no']) ?></code></div>
        <div class="mb-2"><strong>Login Email:</strong> <?= e($login['email'] ?? '—') ?></div>
        <div class="mb-2"><strong>Department:</strong> <?= e($deptName ?: '—') ?></div>
        <div class="mb-2"><strong>Qualification:</strong> <?= e($teacher['qualification'] ?: '—') ?></div>
        <div><strong>Joining Date:</strong> <?= $teacher['joining_date'] ? e(date('d M Y', strtotime($teacher['joining_date']))) : '—' ?></div>
      </div>
      <div class="mt-3">
  ...
  <div class="col-lg-8">
    <div class="cardx">
      ...
      <p class="text-muted" style="font-size:.8rem;margin:-.4rem 0 1rem">Designation, department and qualification are managed by administration. You can update your contact details below.</p>
